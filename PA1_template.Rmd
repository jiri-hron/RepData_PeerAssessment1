...
```
```{r, echo=FALSE}
difMean = abs(mean1 - mean0);
difMedian = abs(median1 - median0);
difTotal = mean(abs(fstepsPerDay - stepsPerDay));
```

As one can see the difference between means is `r difMean` and the difference
between medians is `r difMedian`. Difference between means should be smaller
since median is much more robust statistic than mean. The difference between
the total number of steps per days is `r difTotal`.
